#!/usr/bin/env python3
"""Summarize CIFAR-10 poison experiment results and STL-10 transfer results into Markdown.

This script scans experiment folders under ``poisoned_train_set/cifar10``,
matches the original CIFAR-10 test JSON and the transfer-to-STL10 TXT result
for each run, and writes a Markdown report with:
...
def _normalize_model_name(arch: Optional[str]) -> str:
    if not arch:
        return "unknown"
    if arch.endswith("_cifar10"):
        arch = arch[: -len("_cifar10")]
    return arch
...
def load_records(root: Path) -> Tuple[List[Record], List[MissingRecord]]:
    ...
    for exp_dir in sorted(p for p in root.iterdir() if p.is_dir()):
        ...
        txt_files = sorted(exp_dir.glob("test_stl10_results*.txt"))
        ...
        transfer_acc, transfer_asr, transfer_dataset, model_path = _parse_transfer_txt(txt_files[0])
...
def build_markdown(records: Sequence[Record], missing: Sequence[MissingRecord], root: Path) -> str:
    ...
    transfer_datasets = sorted({rec.transfer_dataset for rec in records if rec.transfer_dataset})

    lines.append("# CIFAR-10 原始测试与迁移测试汇总")
    lines.append("")
    ...
    if transfer_datasets:
        if len(transfer_datasets) == 1:
            lines.append(f"- 迁移目标数据集: `{transfer_datasets[0]}`")
        else:
            lines.append("- 检测到多个迁移目标数据集:")
    ...
    for attack_type in sorted({rec.attack_type for rec in records}):
        ...
        for key in sorted(
            config_grouped.keys(),
            key=lambda items: tuple(
                _to_float(x) if re.fullmatch(r"-?[0-9.]+", x or "") else (x or "")
                for x in items
            ),
        ):
            ...
            head = grouped_rows[0]
            config_rows.append(
                [_format_value(head.params.get(field)) for field, _ in config_columns]
                + [
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize CIFAR-10 original and STL-10 transfer test results.")
    parser.add_argument(
        "--root",
        type=str,
        default="/workspace/backdoor-toolbox-new1/poisoned_train_set/cifar10",
        help="Root directory containing experiment subdirectories.",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="/workspace/backdoor-toolbox-new1/scripts/cifar10_original_vs_transfer_summary.md",
