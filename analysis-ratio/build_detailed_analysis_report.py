...
def load_mode(
    ...
    if apply_asr_filter:
        n_before = len(out)
        out = filter_for_analysis(out, min_asr=min_asr)
        print(
            f"[{mode}] 攻击性筛选 ASR>{min_asr:.0%}: 保留 {len(out)}/{n_before} 条"
        )
...
def build_report(
    ...
    lines.append("## 1. 指标说明\n\n")
    lines.append("- `asr`：源域（训练数据集）测试 ASR，迁移前攻击成功率。\n")
    lines.append("- `transfer_asr`：目标域测试 ASR，迁移后攻击成功率。\n")
    lines.append(
        "- `transfer_rate`：迁移性 = `transfer_asr / (transfer_asr + asr)`（test-ASR 在跨域+同域 ASR 中的占比），取值 (0,1)，越大表示迁移相对越强。\n"
    )
    lines.append(
        f"- 本报告分析子集：**源域 `asr` > {MIN_ASR_FOR_ANALYSIS:.0%}**（提取 CSV 仍为全量）。\n"
    )
    ...
    lines.append("- `transfer_rate` 为比值指标后，与 `asr` 的相关性反映的是「源域越强时迁移保留率如何变化」，需与 `transfer_asr` 一并解读。\n")
...
def main() -> None:
    ...
    apply_filter = not args.no_asr_filter
    df_no_nc = load_mode(
        analysis_dir, "no_nc", min_asr=args.min_asr, apply_asr_filter=apply_filter
    )
    df_nc = load_mode(
        analysis_dir, "nc", min_asr=args.min_asr, apply_asr_filter=apply_filter
    )
