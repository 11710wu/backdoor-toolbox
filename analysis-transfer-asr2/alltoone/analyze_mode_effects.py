...
迁移性 `transfer_rate` = ASRt^2 / ASRs（与 analysis-transfer-asr2 主流程一致）。
"""
    ...
    for mode, root_name in MODE_ROOTS.items():
        ...
        for ds_dir in base.iterdir():
            ...
            for folder in ds_dir.iterdir():
                ...
                source_asr: Optional[float] = None
                for tr_json in folder.glob("test_results*.json"):
                    try:
                        d = json.loads(tr_json.read_text(encoding="utf-8"))
                        if d.get("asr") is not None:
                            source_asr = float(d["asr"])
                            break
                    except Exception:
                        pass
                ...
                for pattern in _transfer_patterns(dataset):
                    for result_file in folder.glob(pattern):
                        ...
                        try:
                            transfer_asr = _parse_transfer_rate(
                                result_file.read_text(encoding="utf-8")
                            )
                        except Exception:
                            transfer_asr = None
                        metric = compute_transfer_rate(transfer_asr, source_asr)
                        if metric is not None:
                            transfer_rates.append(metric)
