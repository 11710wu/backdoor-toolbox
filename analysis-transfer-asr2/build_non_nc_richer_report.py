...
迁移性列 `transfer_rate` = ASRt^2 / ASRs（目标域 ASR 平方 / 源域 ASR），见 transfer_metric.py。
...
def load_defense_results(repo_root: Path, df_no_nc: pd.DataFrame) -> pd.DataFrame:
    ...
    for dataset in base.DATASETS:
        ...
        for folder in sorted(poisoned_root.iterdir()):
            ...
            transfer_asr_by_pval: Dict[float, float] = {}
            asr_by_pval: Dict[float, float] = {}
            ...
            def _collect_transfer(files: List[Path]) -> None:
                for file_path in files:
                    ...
                    if transfer is not None:
                        transfer_asr_by_pval[pval] = transfer
            ...
            for f in folder.glob("test_results*.json"):
                param_info = extract._extract_param_from_test_results(f.name)
                ...
                try:
                    ...
                    if d.get("asr") is not None:
                        asr_by_pval[pval] = float(d["asr"])
                except Exception:
                    pass

            def _transfer_metric(pval: float | None) -> float | None:
                if pval is None:
                    return None
                return compute_transfer_rate(
                    transfer_asr_by_pval.get(pval), asr_by_pval.get(pval)
                )
                    ...
                    rows.append(
                        {
                            ...
                            "tpr": np.nan,
                            "auc": np.nan,
                            "transfer_rate": _transfer_metric(train_param_value),
                            "has_result": False,
                            "folder": folder.name,
                        }
                    )
                    ...
                    rows.append(
                        {
                            ...
                            "transfer_rate": _transfer_metric(file_test_param_value),
...
def build_report(
    ...
    lines.append("- `transfer_rate`：迁移性指标 **ASRt²/ASRs**（目标域 ASR 平方 / 源域 ASR），越大表示相对迁移越强。\n")
