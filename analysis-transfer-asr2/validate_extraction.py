#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""验证提取结果的正确性（迁移性指标为 ASRt^2/ASRs）。支持两种模式：
...
from transfer_metric import compute_transfer_rate
...
def parse_result_files(
    ...
    """解析：stealth、transfer_asr、asr、transfer_rate=ASRt^2/ASRs；include_nc 时含 NC 字段。"""
    res = {
        'stealth_tpr_avg': None, 'stealth_auc_avg': None,
        'transfer_asr': None, 'asr': None, 'transfer_rate': None,
    ...
    try:
        for fn in os.listdir(dir_path):
            if not fn.startswith('test_results') or not fn.endswith('.json'):
                continue
            pinfo = _extract_param_from_test_results(fn)
            pval = pinfo[1] if pinfo else train_param_value
            if pval is None or abs(float(pval) - float(test_param_value)) > 1e-9:
                continue
    ...
    if res['transfer_asr'] is not None and res['asr'] is not None:
        res['transfer_rate'] = compute_transfer_rate(res['transfer_asr'], res['asr'])
...
def check_data_completeness(csv_path, include_nc):
    ...
    required_cols = [
        ...
        ('transfer_asr', '目标域 ASR'),
        ('transfer_rate', '迁移性 ASRt²/ASRs'),
    ]
...
ARCHS = ['resnet18', 'resnet34', 'mobilenet', 'vgg', 'small_cnn']
