...
迁移性指标（与 analysis-testASR 不同）：
  transfer_asr  = 目标域测试 ASR（ASRt，从跨域结果 txt 解析）
  asr           = 源域 ASR（ASRs，从 test_results*.json）
  transfer_rate = ASRt^2 / ASRs  （见 transfer_metric.py）
...
def extract_folder_results(
    ...
    for pval in sorted(test_params.keys()):
        ...
        if transfer_asr is None:
            continue
        transfer_asr = float(transfer_asr)
        asr = d.get('asr')
        if asr is None:
            continue
        asr = float(asr)
        transfer_rate = compute_transfer_rate(transfer_asr, asr)
...
def main():
    parser = argparse.ArgumentParser(
        description='提取隐蔽性、迁移性(ASRt^2/ASRs)、源域 ASR、目标域 ASR，可选 NC 与 S_stealth'
    )
