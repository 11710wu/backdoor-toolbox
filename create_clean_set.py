...
# ========== [Tiny ImageNet 支持] 添加验证集加载 ==========
# Tiny ImageNet: 从 val/ 目录加载验证集
# 用途: 创建 clean_split（用于防御方法验证）和 test_split（用于最终测试）
# 图像尺寸: 使用原始 64×64 尺寸（与训练集一致）
# 注意: 原始图片已经是 64×64，不需要 Resize，直接使用 ToTensor()
elif args.dataset == 'tiny_imagenet':
    data_transform = transforms.Compose([
        transforms.ToTensor(),  # 原始图片已经是 64×64，不需要 Resize
    ])
    clean_set = datasets.ImageFolder(os.path.join(config.tiny_imagenet_dir, 'val'), data_transform)
...
# MNIST-M: 从测试集加载验证集
# 用途: 创建 clean_split（用于防御方法验证）和 test_split（用于最终测试）
# 图像尺寸: 28×28×3（RGB 彩色图）
# 注意: 保存原始图片，不应用数据增强或归一化
    ...
    # 加载 MNIST-M 测试集图像和 MNIST 标签
    mnistm_data = np.load(os.path.join(config.mnistm_dir, 'test.npy'))  # shape: (N, 28, 28, 3), dtype: uint8
    mnist_dataset = MNIST(root=os.path.join(data_dir, 'mnist'), train=False, download=False)
    mnist_labels = mnist_dataset.targets
    
    # 创建简单的数据集，使用 transform 参数（与 Tiny ImageNet 风格一致）
    class SimpleMNISTMDataset(Dataset):
    ...
    clean_set = SimpleMNISTMDataset(mnistm_data, mnist_labels, transform=data_transform)
    img_size = 28
    num_classes = 10
# ========== [MNIST-M 支持] 结束 ==========
