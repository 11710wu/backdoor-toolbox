...
class IMG_Dataset(Dataset):
    def __init__(self, data_dir, label_path, transforms = None, num_classes = 10, shift = False, random_labels = False,
        ...
        self.img_set = None
        # ========== [数据格式判断] 改进判断逻辑 ==========
        # 判断是新版本（imgs 文件）还是旧版本（data/ 目录）
        # 新版本：路径指向一个文件（imgs），例如 'poisoned_train_set/cifar10/.../imgs'
        ...
        if os.path.isfile(data_dir):  # 新版本：imgs 文件
            ...
            # 如果路径不存在，尝试根据路径名判断（向后兼容）
            if 'data' not in self.dir:
                # 可能是新版本，但文件还未创建
                self.img_set = None
            else:
                # 可能是旧版本
        ...
        # 1. 将 tensor 转换为 PIL Image（在 __getitem__ 中）
        # 2. 然后应用完整的 transforms（包括 ToTensor 和 Normalize）
        # 因此不应该移除 ToTensor()，因为我们需要完整的 transforms 流程
        # ========== [修复] 结束 ==========
    ...
    def __getitem__(self, idx):
        ...
        if self.img_set is not None: # if new version (imgs file)
            img = self.img_set[idx]
            # ========== [修复] 保持与 CIFAR-10 一致的处理方式 ==========
            # 新版本数据是 tensor [C, H, W]，值在 [0, 1] 范围内
            # 需要转换为 PIL Image 才能使用 PIL 变换（RandomCrop, RandomHorizontalFlip 等）
            # 然后通过完整的 transforms（包括 ToTensor 和 Normalize）转换回 tensor
            if isinstance(img, torch.Tensor):
                # tensor [C, H, W] → PIL Image
                # ToPILImage() 期望值在 [0, 1] 范围内，与保存的数据格式一致
                # 如果数据是单通道的，需要转换为三通道（对于 MNIST）
                if img.shape[0] == 1:
                    # 单通道转三通道
