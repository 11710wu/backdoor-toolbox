...
from analysis_filters import MIN_ASR_FOR_ANALYSIS
...
def load_defense_results(
    ...
    for dataset in base.DATASETS:
        poisoned_root = poisoned_base / dataset
    ...
    if df.empty:
        raise RuntimeError(f"No defense result records found under {poisoned_base}.")
...
def build_anomaly_tables(
    ...
    high_transfer_high_detection = (
        merged[merged["transfer_rate"] >= 0.7]
        .sort_values(["stealth_mean", "transfer_rate"], ascending=[True, False])
        .head(20)
        .reset_index(drop=True)
    )
    high_stealth_low_transfer = (
        merged[(merged["stealth_mean"] >= 0.65) & (merged["transfer_rate"] < 0.3)]
...
def build_report(
    ...
    lines.append("- `asr`：源域测试 ASR（迁移前）。\n")
    lines.append("- `transfer_asr`：目标域测试 ASR（迁移后）。\n")
    lines.append(
        "- `transfer_rate`：迁移性 = `asr * transfer_asr / (transfer_asr + asr)`；分析子集为源域 `asr > 30%`。\n"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--poisoned-root",
        default="poisoned_train_set1",
        help="投毒实验结果根目录名或绝对路径（默认: poisoned_train_set1）",
    )
    parser.add_argument(
        "--min-asr",
        type=float,
    ...
    parser.add_argument("--no-asr-filter", action="store_true", help="不做 ASR 筛选")
    ...
    poisoned_base = Path(args.poisoned_root)
    if not poisoned_base.is_absolute():
        poisoned_base = repo_root / poisoned_base

    df_no_nc = base.load_mode(
        analysis_dir,
        "no_nc",
        min_asr=args.min_asr,
        apply_asr_filter=not args.no_asr_filter,
    )
    defense_df = load_defense_results(repo_root, df_no_nc, poisoned_base=poisoned_base)
