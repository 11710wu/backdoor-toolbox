...
from analysis_filters import MIN_ASR_FOR_ANALYSIS, filter_for_analysis
...
METRIC_LABELS = {
    ...
    'transfer_asr': 'Target-domain ASR',
    'transfer_rate': 'Transfer ASR product/(sum)',
}

# 比率类指标坐标轴略扩边以免贴边难看
RATE_AXIS_PAD = 0.05
RATE_LIM = (-RATE_AXIS_PAD, 1.0 + RATE_AXIS_PAD)
# 迁移性 asr*test/(test+ASR) 落在 [0, 0.5]
TRANSFER_AXIS_PAD = 0.025
TRANSFER_RATIO_LIM = (-TRANSFER_AXIS_PAD, 0.5 + TRANSFER_AXIS_PAD)
# 色条 / 归一化仍用严格 [0,1]，语义不变
RATE_NORM_LIM = (0.0, 1.0)
TRANSFER_NORM_LIM = (0.0, 0.5)
...
def _ylim_for_metric(ax, col: str) -> None:
    if col == 'transfer_rate':
        ax.set_ylim(*TRANSFER_RATIO_LIM)
    else:
        _ylim_rate(ax)
...
def load_from_csv(
    ...
    apply_asr_filter: bool = True,
) -> pd.DataFrame:
    """从 CSV 加载数据；分析默认仅保留 asr > min_asr（提取 CSV 仍为全量）。"""
    ...
    if apply_asr_filter:
        n0 = len(df)
        df = filter_for_analysis(df, min_asr=min_asr)
        if n0 != len(df):
            print(f"  ASR>{min_asr:.0%} 筛选: {len(df)}/{n0} 条")
    if 'group_id' not in df.columns:
        df['group_id'] = range(1, len(df) + 1)
    return df


def apply_analysis_filters(df: pd.DataFrame, min_asr: float = MIN_ASR_FOR_ANALYSIS) -> pd.DataFrame:
    """分析用：ASR 阈值筛选 + 派生 tradeoff 列。"""
    out = filter_for_analysis(df, min_asr=min_asr)
    if out.empty:
        return out
    out = out.copy()
...
def run_correlation_analysis(df: pd.DataFrame, output_dir: str, arch: str) -> None:
    ...
    labels = [LABEL_STEALTH_TPR, LABEL_STEALTH_AUC, METRIC_LABELS['transfer_rate']]
    ...
    for stealth_col, x_label, name_key in [
        ...
        ax.set_ylabel(METRIC_LABELS['transfer_rate'])
...
def plot_box_by_attack_type_combined_all_archs(
    ...
    for arch, path in found.items():
        df = load_from_csv(path, min_asr=min_asr, apply_asr_filter=apply_asr_filter).copy()
        if df.empty:
            continue
        df['arch'] = arch
        dfs.append(df)
...
def plot_3d_scatter(df: pd.DataFrame, output_path: str, arch: str) -> None:
    ...
    scatter.set_clim(*TRANSFER_NORM_LIM)
...
def analyze_single_arch(
    ...
    """对单个 arch 执行分析"""
    df = load_from_csv(csv_path, min_asr=min_asr, apply_asr_filter=apply_asr_filter)
    print(f"\n[{arch}] 加载 {len(df)} 条数据")
...
def main() -> None:
    ...
    parser.add_argument(
        '--min-asr',
        type=float,
        default=MIN_ASR_FOR_ANALYSIS,
        help='分析时仅保留源域 ASR 高于该阈值的配置（默认 0.3）',
    )
    parser.add_argument(
        '--no-asr-filter',
        action='store_true',
        help='不做 ASR 筛选（使用提取的全量点）',
    )
    args = parser.parse_args()
    apply_asr_filter = not args.no_asr_filter
    ...
    for ds in datasets_to_run:
        ...
        if do_combined and len(discover_arch_csvs(data_dir, suffix=suffix, dataset=ds)) >= 2:
            os.makedirs(output_dir, exist_ok=True)
            plot_scatter_combined_all_archs(
                data_dir, output_dir, suffix=suffix, dataset=ds,
                min_asr=min_asr, apply_asr_filter=apply_asr_filter,
            )
        if do_combined_box and len(discover_arch_csvs(data_dir, suffix=suffix, dataset=ds)) >= 2:
            os.makedirs(output_dir, exist_ok=True)
            plot_box_by_attack_type_combined_all_archs(
                data_dir, output_dir, suffix=suffix, dataset=ds,
                min_asr=min_asr, apply_asr_filter=apply_asr_filter,
            )
