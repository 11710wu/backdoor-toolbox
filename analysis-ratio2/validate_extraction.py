...
from extract_all_results import (
    ...
    _extract_param_from_test_results,
    _compute_transfer_ratio,
    DEFAULT_POISONED_ROOT,
)
...
def parse_result_files(
    ...
    """解析：stealth_tpr_avg, stealth_auc_avg, transfer_asr, transfer_rate(联合ASR), asr；include_nc 时含 nc 字段"""
    res = {'stealth_tpr_avg': None, 'stealth_auc_avg': None, 'transfer_asr': None, 'transfer_rate': None,
           'asr': None, 'nc_max_anomaly_index': None, 'nc_is_poisoned': None}
    ...
    # 2b. 源域 ASR：test_results*.json（与 extract_all_results 相同规则）
    # 无参数后缀的 test_results_seed=2333.json 对应 train_param_value，而非 test 扫参文件。
    try:
        for f in sorted(os.listdir(dir_path)):
            if not f.startswith('test_results') or not f.endswith('.json'):
                continue
            param_info = _extract_param_from_test_results(f)
            pval = param_info[1] if param_info else train_param_value
            if pval is None or test_param_value is None:
                continue
            if abs(float(pval) - float(test_param_value)) > 1e-9:
                continue
            try:
    ...
    res['transfer_rate'] = _compute_transfer_ratio(res.get('transfer_asr'), res.get('asr'))
...
def check_data_completeness(csv_path, include_nc):
    ...
    required_cols = [
        ...
        ('transfer_asr', '目标域ASR'),
        ('transfer_rate', '迁移性 asr*test/(test+asr)'),
        ('asr', '源域ASR'),
    ]
...
def main():
    ...
    if not os.path.isabs(poisoned_root):
        poisoned_root = os.path.join(project_root, poisoned_root)
    datasets_loop = ['cifar10', 'tiny_imagenet', 'mnistm'] if args.dataset == 'all' else [args.dataset]

    print(f"[数据源] {poisoned_root}\n")
    ...
    for ds in datasets_loop:
        base_dir = os.path.join(poisoned_root, ds)
