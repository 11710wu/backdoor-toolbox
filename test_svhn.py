#!/usr/bin/env python3
"""Evaluate a SYN-trained model on the fixed cropped-SVHN target subset."""
...
def main():
    ...
    loader = torch.utils.data.DataLoader(target_set, batch_size=128, shuffle=False,
                                         num_workers=4, pin_memory=True, worker_init_fn=tools.worker_init)
    ...
    counts = evaluate_clean_and_asr(model, loader, poison_transform, config.target_class['syn'])
    results = {
        'source_dataset': 'syn', 'target_dataset': 'svhn_test',
        'poison_type': args.poison_type, 'poison_rate': args.poison_rate,
        'seed': args.seed, 'poison_seed': args.poison_seed,
        'target_clean_correct': counts['clean_correct'], 'target_clean_total': counts['clean_total'],
        'target_clean_acc': counts['clean_acc'], 'transfer_success': counts['asr_success'],
        'transfer_eligible': counts['asr_eligible'], 'target_transfer_asr': counts['asr'],
        'model_path': model_path, 'target_manifest': 'svhn_test_10k_manifest.csv',
        'bn_recalibration': False, 'timestamp': datetime.datetime.now().isoformat(),
    }
    output = os.path.join(poison_dir, f'test_svhn_results_seed={args.seed}.json')
    with open(output, 'w', encoding='utf-8') as handle:
        json.dump(results, handle, indent=2, sort_keys=True)
    print(json.dumps(results, indent=2, sort_keys=True))
    print(f'[SVHN] results saved to {output}')
