...
# 所有数据集统一使用 CosineAnnealingLR（余弦退火学习率）
...
for epoch in range(1,epochs+1):
    ...
    if args.dataset != 'ember':
        if epoch % 20 == 0:
            clean_acc, asr = tools.test(model=model, test_loader=test_set_loader, poison_test=True,
...
# ========== [结果保存] 保存清理后训练的最终结果到JSON文件 ==========
if args.dataset != 'ember':
    # 如果最后一个epoch没有测试，现在测试一次
    if epochs % 20 != 0:
        final_clean_acc, final_asr = tools.test(model=model, test_loader=test_set_loader, poison_test=True,
                       poison_transform=poison_transform, num_classes=num_classes, source_classes=source_classes)
    ...
    results = {
        ...
        'model_path': supervisor.get_model_dir(args, cleanse=True),
        'training_type': 'cleansed',
    }
    ...
    # 保存结果文件到模型所在目录
    model_dir = os.path.dirname(supervisor.get_model_dir(args, cleanse=True))
    results_file = os.path.join(model_dir, f'train_cleansed_{args.cleanser}_seed={args.seed}.json')
    with open(results_file, 'w') as f:
        json.dump(results, f, indent=4)
    
    print(f"\n{'='*60}")
    print(f"[Results Saved] {results_file}")
    print(f"  Cleanser:  {args.cleanser}")
