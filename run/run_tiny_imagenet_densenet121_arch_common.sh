...
# Shared runner for Tiny-ImageNet DenseNet121 architecture experiments.
# Entry-point scripts set ATTACK_LIST and PREPARE_CLEAN.
...
PREPARE_UPGD_RAW_BASE="${PREPARE_UPGD_RAW_BASE:-${PREPARE_CLEAN}}"
...
ATTACK_LIST="${ATTACK_LIST:?Set ATTACK_LIST, e.g. 'basic blend'}"
...
STANDARD_POISON_RATES="${STANDARD_POISON_RATES:-0.005 0.010 0.050}"
BELT_POISON_RATES="${BELT_POISON_RATES:-0.010 0.020 0.100}"
# Tiny-ImageNet ResNet18 baseline currently has SIG/UPGD clean-label rows at 0.001 and 0.005.
# Override this env var only if you intentionally want extra unmatched rates.
SIG_UPGD_POISON_RATES="${SIG_UPGD_POISON_RATES:-0.001 0.005}"
...
UPGD_RAW_BASE_DIR="${UPGD_RAW_BASE_DIR:-${POISONED_TRAIN_SET_ROOT}/${DATASET}/upgd_raw_base_0.000_poison_seed=2333_arch=densenet121_tiny_imagenet}"
UPGD_CLEAN_MODEL_PATH="${UPGD_CLEAN_MODEL_PATH:-${UPGD_RAW_BASE_DIR}/upgd_raw_base_densenet121_tiny_imagenet.pt}"

LOG_DIR="${LOG_DIR:-logs}"
mkdir -p "$LOG_DIR"
TIMESTAMP="$(date +"%Y%m%d_%H%M%S")"
RUN_NAME="${RUN_NAME:-run_tiny_imagenet_densenet121_arch}"
...
poison_rates_for_attack() {
  case "$1" in
    "SIG"|"upgd") echo "${SIG_UPGD_POISON_RATES}" ;;
    "belt") echo "${BELT_POISON_RATES}" ;;
    *) echo "${STANDARD_POISON_RATES}" ;;
  esac
}
...
strength_label() {
  case "$1" in
    "SIG") echo "delta=$2" ;;
    "WaNet") echo "s=$2" ;;
    "belt"|"basic"|"blend"|"adaptive_patch"|"adaptive_blend") echo "alpha=$2" ;;
    "upgd") echo "eps=$2" ;;
    *) echo "strength=$2" ;;
  esac
}
...
run_config_phase() {
  ...
  for attack in "${ATTACKS[@]}"; do
    for rate in $(poison_rates_for_attack "$attack"); do
...
echo "============================================================"
echo "${RUN_TITLE:-Tiny-ImageNet DenseNet121 architecture experiment}"
echo "============================================================"
...
echo "std rates    : ${STANDARD_POISON_RATES}"
echo "belt rates   : ${BELT_POISON_RATES}"
echo "sig/upgd rates: ${SIG_UPGD_POISON_RATES}"
...
if [ "$PREPARE_CLEAN" = "1" ]; then
  ...
  run_command \
    ...
    "Train normalized clean DenseNet121 model"
  ...
  if [ "$PREPARE_UPGD_RAW_BASE" = "1" ]; then
    if [ -f "$UPGD_CLEAN_MODEL_PATH" ]; then
      ...
      run_command \
        ...
        "Train raw-input clean DenseNet121 model for UPGD"
    fi
  fi
fi
...
if [ "$RUN_TARGET" = "1" ]; then
  run_config_phase \
    "4. ImageNetV2-tiny transfer testing" \
    "test_tiny_target_domain.py" \
    "ImageNetV2-tiny transfer" \
    "-source_dataset=${DATASET} -target_domain_dir=${TARGET_DOMAIN_DIR}"
...
echo
echo "============================================================"
echo "Tiny-ImageNet DenseNet121 architecture experiment finished."
