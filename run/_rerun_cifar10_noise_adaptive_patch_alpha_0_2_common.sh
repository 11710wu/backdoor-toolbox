#!/usr/bin/env bash
#
# Shared driver for CIFAR-10 noise adaptive-patch alpha=0.2 reruns.
# Set MODEL, ARCH_NAME, PART_TITLE, EXPECTED_DIRS before sourcing.
#
# For SmallCNN, set OUTPUT_ROOT=poisoned_train_set1/cifar10 and MOVE_TO_SET1=0
# (pipeline still writes via poisoned_train_set; post-move optional).

set +e

: "${MODEL:?Set MODEL}"
: "${ARCH_NAME:?Set ARCH_NAME}"
...
PART_SLUG="${PART_SLUG:-${MODEL}}"
ERROR_LOG="${ERROR_LOG:-${LOG_DIR}/rerun_cifar10_noise_adaptive_patch_alpha_0_2_${PART_SLUG}_${TIMESTAMP}.log}"
...
safe_cleanup_old_alpha() {
  echo
  echo "----- Safe cleanup: CIFAR-10 noise adaptive_patch alpha=0.200 (${MODEL}) -----"
  local base="poisoned_train_set/${DATASET}"
  local paths=()
  local path name

  [ -d "$base" ] || return 0
  for path in "$base"/adaptive_patch_*_alpha=0.200_*_noise=*_arch="${ARCH_NAME}"; do
    [ -d "$path" ] || continue
    name="$(basename "$path")"
    case "$path" in
      poisoned_train_set/${DATASET}/adaptive_patch_*_alpha=0.200_*_noise=*_arch=${ARCH_NAME}) ;;
      *)
    ...
    case "$name" in
      adaptive_patch_*_alpha=0.200_*_noise=*_arch=${ARCH_NAME}) paths+=("$path") ;;
      *)
...
move_smallcnn_results_to_set1() {
  [ "$MOVE_TO_SET1" = "1" ] || return 0
  [ "$MODEL" = "small_cnn" ] || return 0

  local src_base="poisoned_train_set/${DATASET}"
  local dst_base="poisoned_train_set1/${DATASET}"
  mkdir -p "$dst_base"

  echo
  echo "----- Move SmallCNN results to poisoned_train_set1 -----"
  local path name moved=0
  shopt -s nullglob
  for path in "${src_base}"/adaptive_patch_*_alpha=0.200_*_noise=*_arch="${ARCH_NAME}"; do
    [ -d "$path" ] || continue
    name="$(basename "$path")"
    if [ "$DRY_RUN" = "1" ]; then
      echo "[DRY_RUN MOVE] ${name}"
      continue
    fi
    mv -- "$path" "${dst_base}/${name}"
    moved=$((moved + 1))
  done
  shopt -u nullglob
  echo "Moved ${moved} directories to ${dst_base}"
}

echo "============================================================"
echo "CIFAR-10 noise Adaptive-Patch alpha=0.2 rerun: ${PART_TITLE}"
echo "============================================================"
echo "workload     : ${EXPECTED_DIRS} result dirs (~${EXPECTED_DIRS} GPU-hours @ 1h/dir)"
...
echo "move to set1 : ${MOVE_TO_SET1}"
...
echo
echo "CIFAR-10 noise Adaptive-Patch alpha=0.2 ${PART_TITLE} rerun script finished."
