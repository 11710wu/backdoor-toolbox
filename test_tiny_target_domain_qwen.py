"""Test backdoor transferability on the Qwen-generated Tiny target-domain dataset.

This script mirrors ``test_tiny_target_domain.py`` but defaults to the Qwen
full organized target-domain dataset produced by
``scripts/organize_tiny_target_domain.py`` and writes results to a separate
file name so the original ImageNetV2 target-domain results are NOT overwritten.

Key design choices (kept identical to test_tiny_target_domain.py):
- source_dataset defaults to ``tiny_imagenet``
- model loading, poison_transform, ASR definition are the same
- Output file: ``test_tiny_target_domain_qwen_results*.txt``
"""
...
# ============================================================================
# Core evaluation (identical logic to test_tiny_target_domain.test_target_domain_model)
# ============================================================================
...
def test_target_domain_model(
    ...
    print(f"执行 Tiny Target Domain (Qwen) 迁移测试: 攻击类型={poison_type}, 源数据集={source_dataset}")
    ...
    # -- Clean accuracy --
    print("\n=== 计算准确率（Tiny Target Domain Qwen，不加触发器）===")
    acc_correct = 0
    acc_total = 0
...
def _save_example(original, poisoned, attack_type, model_dir, source_dataset):
    save_dir = os.path.join(model_dir, "tiny_target_domain_qwen_examples")
...
# ============================================================================
# Poison transform (identical to test_tiny_target_domain.create_poison_transform)
# ============================================================================
...
def _resolve_legacy_model_path(model_path: str) -> str:
    """兼容 SIG / UPGD 的实际保存目录命名。

    ``get_poison_set_dir`` 会为 SIG / UPGD 追加 ``_mode=all2one`` 段，但实际训练
    保存的目录名并不包含该段（例如 ``SIG_0.001_delta=12_f=6_poison_seed=...`` 与
    ``upgd_0.001_eps=10.0_constraint=Linf_steps=100_mult=5_poison_seed=...``）。
    当默认路径不存在时，回退到去掉 ``_mode=all2one`` 的等价路径。
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Tiny Target Domain (Qwen) 后门迁移测试"
    )

    # Basic params (same as test_tiny_target_domain.py)
    ...
    if args.dataset != "tiny_imagenet":
        raise ValueError(
            f"test_tiny_target_domain_qwen.py requires -dataset tiny_imagenet, got: {args.dataset}"
        )
    ...
    # ---- Save original params & get model path (same logic as test_tiny_target_domain.py) ----
    ...
    # 兼容 SIG / UPGD 的实际保存目录（不含 _mode=all2one 段）：
    # 默认路径不存在时回退到去掉该段的目录。
    model_path = _resolve_legacy_model_path(model_path)
    # 对 UPGD，delta 文件与模型同目录；显式把回退后的目录传给 poison_transform，
    # 使其在 get_poison_set_dir 推断的（带 _mode=all2one 的）路径缺失时能正确读取 delta。
    ...
    # ---- Load model weights (same as test_tiny_target_domain.py) ----
    ...
    # ---- Save results (separate file name to avoid overwriting ImageNetV2 results) ----
    if args.poison_type == "SIG":
        if args.test_delta is not None:
            base = f"test_tiny_target_domain_qwen_results_test_delta={args.test_delta}.txt"
        else:
            base = f"test_tiny_target_domain_qwen_results_delta={args.delta}.txt"
    elif args.poison_type == "WaNet":
        if args.test_s is not None:
            base = f"test_tiny_target_domain_qwen_results_test_s={args.test_s}.txt"
        else:
            base = f"test_tiny_target_domain_qwen_results_s={args.s}.txt"
    elif args.poison_type in ("blend", "adaptive_blend", "adaptive_patch", "basic", "clean_label"):
        if args.test_alpha is not None:
            base = f"test_tiny_target_domain_qwen_results_test_alpha={args.test_alpha}.txt"
        else:
            base = "test_tiny_target_domain_qwen_results.txt"
    else:
        base = "test_tiny_target_domain_qwen_results.txt"
    ...
    with open(result_path, "w", encoding="utf-8") as f:
        f.write("=== Tiny Target Domain (Qwen) 迁移测试结果 ===\n")
        f.write(f"源数据集: {source_dataset}\n")
        f.write(f"目标数据集: tiny_target_domain_qwen\n")
