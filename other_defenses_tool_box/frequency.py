...
class Frequency(BackdoorDefense):
    ...
    def detect(self, inspect_correct_predition_only=True, noisy_test=False):
        ...
        # ========== [Frequency检测结果保存修改] 开始 ==========
        # 将Frequency检测结果保存到中毒数据集文件夹
        # 目的: 方便后续统一评估不同防御方法的效果
        poison_set_dir = supervisor.get_poison_set_dir(args)
        frequency_results = {
            'defense_method': 'Frequency (Frequency Domain Detection)',
            'auc': auc_value,  # Area Under Curve (ROC曲线下面积)
        }
        
        # 保存结果到JSON文件
        frequency_results_path = os.path.join(poison_set_dir, 'frequency_defense_results.json')
        with open(frequency_results_path, 'w') as f:
            json.dump(frequency_results, f, indent=4)
        
        # 打印Frequency检测结果摘要
        print(f"\n{'='*50}")
        print(f"[Frequency Detection Results]")
        print(f"TPR: {tp / (tp + fn) * 100:.2f}%")
        print(f"FPR: {fp / (tn + fp) * 100:.2f}%")
        print(f"AUC: {auc_value:.4f}")
        print(f"Results saved to: {frequency_results_path}")
        print(f"{'='*50}\n")
        # ========== [Frequency检测结果保存修改] 结束 ==========
