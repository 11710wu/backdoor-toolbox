...
class ABL(BackdoorDefense):
    ...
    def detect(self):
        # ========== [ABL结果保存修改] 开始 ==========
        # 在防御前测试原始中毒模型,获取防御前的CA和ASR用于后续保存
        # test函数会打印结果并返回(ca, asr),我们捕获返回值
        ca_before, asr_before = test(self.model, test_loader=self.test_loader, poison_test=True, poison_transform=self.poison_transform, num_classes=self.num_classes, source_classes=self.source_classes, all_to_all=('all_to_all' in self.args.dataset))
        # 保存为类属性,供train_unlearn方法使用
        self.ca_before = ca_before
        self.asr_before = asr_before
        # ========== [ABL结果保存修改] 结束 ==========
    ...
    def train_isolation(self):
        ...
        # ========== [保存isolation checkpoint - 必须] ==========
        # 原因: train_unlearn方法需要加载isolation阶段的checkpoint
        # save isolated model
    ...
    def train_unlearn(self):
        ...
        # ========== [ABL结果保存修改] 开始 ==========
        # 获取防御后的最终结果(test函数会打印结果并返回ca和asr)
        ca_after, asr_after = test(model_ascent, test_loader=self.test_loader, poison_test=True, poison_transform=self.poison_transform, num_classes=self.num_classes, source_classes=self.source_classes, all_to_all=('all_to_all' in self.args.dataset))
        
        # 保存ABL防御结果到中毒数据集文件夹
        # 目的: 方便后续统一评估不同防御方法的效果
        if hasattr(self, 'ca_before') and hasattr(self, 'asr_before'):
            poison_set_dir = supervisor.get_poison_set_dir(args)
            abl_results = {
                'defense_method': 'ABL (Anti-Backdoor Learning)',
                'isolation_ratio': self.isolation_ratio,  # 隔离样本比例
                'gradient_ascent_type': self.gradient_ascent_type,  # 梯度上升类型(LGA/Flooding)
                'clean_acc_before': float(self.ca_before * 100),  # 防御前干净数据准确率
                'asr_before': float(self.asr_before * 100),  # 防御前攻击成功率
                'clean_acc_after': float(ca_after * 100),  # 防御后干净数据准确率
                'asr_after': float(asr_after * 100),  # 防御后攻击成功率
                'asr_reduction': float((self.asr_before - asr_after) * 100),  # ASR绝对降低值
                'asr_reduction_rate': float((self.asr_before - asr_after) / self.asr_before * 100) if self.asr_before > 0 else 0.0,  # ASR相对降低率
            }
            
            # 保存结果到JSON文件
            abl_results_path = os.path.join(poison_set_dir, 'abl_defense_results.json')
            with open(abl_results_path, 'w') as f:
                json.dump(abl_results, f, indent=4)
        # ========== [ABL结果保存修改] 结束 ==========
        
        # ========== [不保存checkpoint和最终模型] ==========
        # 原因: checkpoint仅用于断点续训,不需要保存
        # 最终修复后的模型仅用于评估ASR降低效果,不需要保存
        # 如果需要保存,取消下面几行的注释
