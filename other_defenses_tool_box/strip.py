...
class STRIP(BackdoorDefense):
    ...
    def __init__(self, args, strip_alpha: float = 0.5, N: int = 64, defense_fpr: float = 0.05, batch_size=128):
        ...
        # 导致：test 数据（[0,1] 空间）+ train 数据（normalized 空间）→ 数据空间不匹配
        # 修复：显式传入 data_transform，确保 train_loader 和 test_loader 使用相同的 transform
        # ========== [修复] 结束 ==========
        self.train_loader = generate_dataloader(dataset=self.dataset,
                                                dataset_path=config.data_dir,
                                                batch_size=batch_size,
                                                split='train',
                                                data_transform=self.data_transform,  # ✅ 使用与 test_loader 相同的 transform
                                                shuffle=True,
                                                drop_last=True)
    ...
    def detect(self, inspect_correct_predition_only=True):
        ...
        clean_entropy = []
        poison_entropy = []
        for _input, _label in tqdm(test_set_loader, desc="计算测试集熵值"):
            _input, _label = _input.cuda(), _label.cuda()
        ...
        # ========== [修改结束] ==========
        strip_results = {
            'defense_method': 'STRIP (STRong Intentional Perturbation)',
        ...
        strip_results_path = os.path.join(poison_set_dir, 'strip_defense_results.json')
        with open(strip_results_path, 'w') as f:
            json.dump(strip_results, f, indent=4)
        
        # 打印STRIP检测结果摘要
        print(f"\n{'='*50}")
        print(f"[STRIP Detection Results]")
        print(f"Clean Entropy Median: {clean_entropy.median():.4f}")
        print(f"Poison Entropy Median: {poison_entropy.median():.4f}")
        print(f"TPR (中毒样本检测准确率): {tpr_value:.2f}%")
        print(f"FPR (假阳性率): {fpr_value:.2f}%")
        print(f"AUC: {auc_value:.4f}")
        print(f"Results saved to: {strip_results_path}")
        print(f"{'='*50}\n")
        # ========== [STRIP检测结果保存修改] 结束 ==========
    ...
    def superimpose(self, _input1: torch.Tensor, _input2: torch.Tensor, alpha: float = None):
        """
        将两个输入图像叠加（用于 STRIP 防御方法）
        
        Args:
            _input1: 第一个输入图像 tensor（通常是测试图像）
            _input2: 第二个输入图像 tensor（通常是训练图像，用于叠加）
            alpha: 叠加系数，如果为 None 则使用 self.strip_alpha
        ...
        _input2 = _input2[:_input1.shape[0]]  # 确保 batch size 一致
        
        # ========== [Tiny ImageNet 支持] 处理尺寸不匹配问题 ==========
