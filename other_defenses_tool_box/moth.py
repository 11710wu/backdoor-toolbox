...
_mean = {
    ...
    'tiny_imagenet': [0.4802, 0.4481, 0.3975],
    'mnistm':   [0.46, 0.46, 0.46],
}
...
_std = {
    ...
    'tiny_imagenet': [0.2302, 0.2265, 0.2262],
    'mnistm':   [0.23, 0.23, 0.23],
}
...
class Trigger:
    ...
    def generate(self, pair, x_set, y_set, attack_size=100, steps=1000,
        ...
        if init_p is None:
            ...
            init_pattern = init_p
        # 转换到 tanh 空间！
        # 因为后续我们要用 tanh 函数把它们限制在 [0, 1] 区间内，且 tanh 是可导的
        ...
        for step in range(self.steps):
            ...
            # 当准确率超过阈值且正则化项小于当前最佳正则化项时，更新最佳mask和pattern
            if avg_acc >= self.asr_bound and avg_loss_reg < reg_best:
                ...
                init_mask    = torch.arctanh((init_mask - 0.5)\
                                                * (2 - self.epsilon))
                #优化器优化的是 mask_tensor (定义域(−∞,+∞)),实际使用的mask值域维持在[0,1]区间内
                ...
                init_pattern = torch.arctanh((init_pattern - 0.5)\
                                                * (2 - self.epsilon))
                #将添加噪声后的 mask 和 pattern 重新赋值给优化器的变量
                with torch.no_grad():
            ...
            # adjust loss weight
            #为了避免频繁调整，设置了 patience 参数，只有超过阈值时才进行增大或者减小
            #cost变大，增大正则化项，导致触发器变小
...
class TriggerCombo:
    ...
    def generate(self, pair, x_set, y_set, m_set, attack_size=100, steps=1000,
        ...
        # set loss function and optimizer
        criterion = torch.nn.CrossEntropyLoss(reduction='none')
        #设置优化器允许优化的目标为 mask_tensor 和 pattern_tensor
        optimizer = torch.optim.Adam([self.mask_tensor, self.pattern_tensor],
                                     lr=0.1, betas=(0.5, 0.9))
        ...
        for step in range(self.steps):
            ...
            for idx in range(x_set.shape[0] // self.batch_size):
                ...
                #两个触发器分开优化如果 m_batch 是 1（Source → Target 任务），前半部分有效
...
class moth(BackdoorDefense):
    ...
    def __init__(self, args, pair='0-0', type='nat', suffix='nat', batch_size=128, lr=1e-3, epochs=2, data_ratio=1.0, warm_ratio=0.5, portion=0.1):
        ...
        if args.dataset == 'cifar10':
            ...
            self.mean = torch.FloatTensor([0.4802, 0.4481, 0.3975])
            self.std = torch.FloatTensor([0.2302, 0.2265, 0.2262])
        elif args.dataset == 'mnistm':
            self.mean = torch.FloatTensor([0.46, 0.46, 0.46])
            self.std = torch.FloatTensor([0.23, 0.23, 0.23])
        else: raise NotImplementedError()

    def detect(self):
        args = self.args
        
        if args.dataset in ['cifar10', 'gtsrb', 'mnistm', 'tiny_imagenet']:
    ...
    def moth_core(self, args, model, train_loader, test_loader, poison_set_dir):
        ...
        print(f"\n{'='*50}")
        print("Evaluating model performance BEFORE MOTH defense...")
        ca_before, asr_before, _ = val_atk(self.args, model)
        print(f"[MOTH Before Defense] Clean Acc: {ca_before:.2f}%, ASR: {asr_before:.2f}%")
        print(f"{'='*50}\n")
        ...
        # 记录距离变化的矩阵
        mat_univ  = np.zeros((num_classes, num_classes)) # warmup distance热身阶段的初始距离
        mat_size  = np.zeros((num_classes, num_classes)) # trigger size
        mat_diff  = np.zeros((num_classes, num_classes)) # distance improvement防御效果的提升
                ...
                if (WARMUP and step % warmup_steps == 0) or \
                    ...
                    if WARMUP:
                        ...
                        if np.random.rand() < 0.3:
                            ...
                            #随着训练步数 step 的增加，alpha 越来越大
                            #univ_sum是热身时候类别对的初始距离，diff_sum是防御效果的提升
        ...
        # ========== [新增] 评估防御后的性能 ==========
        print(f"\n{'='*50}")
        print("Evaluating model performance AFTER MOTH defense...")
        ca_after, asr_after, _ = val_atk(self.args, model)
        print(f"[MOTH After Defense] Clean Acc: {ca_after:.2f}%, ASR: {asr_after:.2f}%")
        
        # 保存详细结果到 JSON (类似 Neural Cleanse)
        moth_results = {
            'defense_method': 'MOTH',
            'dataset': args.dataset,
            'clean_acc_before': float(ca_before),
            ...
            'asr_reduction_rate': float((asr_before - asr_after) / asr_before * 100) if asr_before > 0 else 0.0,
        }
        
        results_path = os.path.join(poison_set_dir, 'moth_defense_results.json')
        with open(results_path, 'w') as f:
            json.dump(moth_results, f, indent=4)
