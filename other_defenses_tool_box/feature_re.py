...
class FeatureRE(BackdoorDefense):
    ...
    def __init__(self, args, wp_epochs=100, epochs=400):
        ...
        # Tiny-ImageNet 图像更大(64x64)、类别多(200)，降低 batch_size 避免 CUDA OOM
        self.batch_size = 24 if args.dataset == 'tiny_imagenet' else 100
        ...
        # Determine dataset properties for UNet configuration
        if hasattr(self.dataset, 'input_channels'):
            input_channels = self.dataset.input_channels
        else:
            # Fallback for datasets that expose channel via sample shape (e.g. from dummy sample)
            try:
                # Assuming dataset[0][0] is a tensor of shape (C, H, W)
                dummy_input, _ = self.train_set[0]
                input_channels = dummy_input.shape[0]
            except:
                input_channels = 3

        # Configure UNet blocks: unify to 4 blocks for all datasets (except MNIST-M)
        if args.dataset == 'mnistm':
            unet_blocks = 3
        else:
            unet_blocks = 4 

        self.AE = UNet(n_channels=input_channels, num_classes=input_channels, base_filter_num=32, num_blocks=unet_blocks)
        self.AE.cuda()
        self.AE.train()
        self._EPSILON = 1e-7
        self.init_mask, self.feature_shape = self.get_mask()
        self.mask_tanh = nn.Parameter(torch.tensor(self.init_mask).cuda())
        self.all_features, self.weight_map_class = self.get_range()
        # 确保 all_features 在 CUDA 上
        self.all_features = self.all_features.cuda()
        self.weight_map_class = [w.cuda() for w in self.weight_map_class]
    ...
    def detect(self):
        ...
        # 外层进度条：显示 epoch 进度
        epoch_pbar = tqdm(range(self.epchs), desc="Training", unit="epoch")
        for epoch in epoch_pbar:
            ...
            # 内层进度条：显示每个 epoch 内的 batch 进度
            batch_pbar = tqdm(self.re_dataloader, 
                             desc=f"Epoch {epoch+1}/{self.epchs}",
                             leave=False,
                             unit="batch")
            for batch_idx, (inputs, labels) in enumerate(batch_pbar):
                ...
                if epoch < self.wp_epochs:
                    ...
                    # 确保 mask 和 features_ori 在同一设备上
                    mask = mask.to(features_ori.device)
                    loss_std = (features_ori * mask).std(0).sum()
                    loss_std = loss_std / (torch.norm(mask, 1))
            ...
            # 更新 epoch 进度条信息
            epoch_info = "ASR: {:.1f}% | CE: {:.4f} | Dist: {:.4f} | Mixed: {:.4f}".format(
                true_pred * 100.0 / total_pred, avg_loss_ce, avg_loss_dist, 
                mixed_value_best.item() if hasattr(mixed_value_best, 'item') else float(mixed_value_best)
            )
        ...
        # Detection Logic based on fixed threshold (FeatureRE source code style)
        mixed_value_threshold = -0.75 # Default threshold from FeatureRE config
        # 转为 Python 标量，否则 JSON 无法序列化 Tensor
        mv_best = mixed_value_best.item() if hasattr(mixed_value_best, 'item') else float(mixed_value_best)
        is_poisoned = bool(mv_best < mixed_value_threshold)
        
        results = {
            'mixed_value_best': mv_best,
    ...
    def get_dataloader_label_remove(self):
        ...
        dataloader_class = torch.utils.data.DataLoader(class_dataset, batch_size=self.batch_size, pin_memory=True, shuffle=True)
    ...
    def forward_ae_mask_p(self, x):
        ...
        reference_features = reference_features.to(features.device)
