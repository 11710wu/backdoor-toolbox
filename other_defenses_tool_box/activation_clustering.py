...
class AC(BackdoorDefense):
    ...
    def detect(self, inspect_correct_predition_only=True, noisy_test=False):
        ...
        for target_class in range(self.num_classes):
            ...
            print('[class-%d] silhouette_score = %f, outliers = %d' % (target_class, score, len(outliers)))
        ...
        print("Actual Target Class (from config):", config.target_class.get(args.dataset, 'N/A'))
        ...
        # ========== [修改结束] ==========
        ac_results = {
            'defense_method': 'AC (Activation Clustering)',
            'suspicious_class': int(suspicious_class),  # AC识别的可疑类别
            'silhouette_scores': [float(s) for s in class_scores],  # 每个类别的轮廓系数
        ...
        ac_results_path = os.path.join(poison_set_dir, 'ac_defense_results.json')
        with open(ac_results_path, 'w') as f:
            json.dump(ac_results, f, indent=4)
        
        # 打印AC检测结果摘要
        print(f"\n{'='*50}")
        print(f"[AC Detection Results]")
        print(f"Suspicious Class: {suspicious_class}")
        print(f"TPR (中毒样本检测准确率): {tpr_value:.2f}%")
        print(f"FPR (假阳性率): {fpr_value:.2f}%")
        print(f"AUC: {auc_value:.4f}")
        print(f"Results saved to: {ac_results_path}")
        print(f"{'='*50}\n")
        # ========== [AC检测结果保存修改] 结束 ==========
